...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $formData['predmet'] = trim($_POST['predmet'] ?? '');
    $formData['priorita'] = trim($_POST['priorita'] ?? '');
    ...
    if ($formData['predmet'] === '')
        {
            $chyby['predmet']= 'predmet je povinny';
        }
    elseif(!array_key_exists($formData['predmet'], $predmety))
        {
            $chyby['predmet']='neplatny predmet';
        }

    if ($formData['priorita'] === '')
        {
            $chyby['priorita']= 'priorita je povinna';
        }
    elseif(!array_key_exists($formData['priorita'], $priority))
        {
            $chyby['priorita']='neplatna priorita';
        }
...
elseif (mb_strlen($formData['zprava']) < 10) 
    {
    $chyby['zprava'] = 'Zpráva musí mít alespoň 10 znaků!';
    } 
elseif (mb_strlen($formData['zprava']) > 1000)
    {
    $chyby['zprava'] = 'Zpráva může mít maximálně 1000 znaků!';
    }
