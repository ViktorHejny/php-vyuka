...
$formData = [
    ...
    'barva' => $profil['#4F5B93'], // TODO: načti z $profil
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $formData['barva'] = $_POST['barva'] ?? '#4F5B93';
    ...
    elseif(mb_strlen($formData['jmeno'])<2)
        {
            $chyby['jmeno'] = 'Jméno musí mit alespoň 2 znaky.';
        }
    ...
    elseif(mb_strlen($formData['prijmeni'])<2)
        {
            $chyby['prijmeni'] = 'Přijmení musí mit alespoň 2 znaky.';
        }
    ...
    elseif(!filter_var($formData['email'], FILTER_VALIDATE_EMAIL))
        {
            $chyby['email'] = 'Zadej platný emial.';
        }
    else
        {
    ...
    if($formData['prezdivka'] == '')
        {
            $chyby['prezdivka'] = 'Prezdivka je povinná';
        }
    elseif (mb_strlen($formData['prezdivka']) < 3 || mb_strlen($formData['prezdivka']) > 20)
        {
            $chyby['prezdivka'] = 'Minimalne 3 znaky, max 20.';
        }
    elseif (!preg_match('/^[a-zA-Z0-9_]+$/', $formData['prezdivka']))
        {
            $chyby['prezdivka'] = 'Jen písmena, čísla a podtržítka.';
        }       

    // TODO: Validace třídy – whitelist: '', '1.A', '1.B', '2.A', '2.B', '3.A', '3.B', '4.A', '4.B'

    $Tridy = [ '', '1.A', '1.B', '2.A', '2.B', '3.A', '3.B', '4.A', '4.B'];
    if(!in_array($formData['pohlavi'], $Tridy, true))
        {
